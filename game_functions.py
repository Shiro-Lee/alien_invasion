...
def save_high_score(stats):
    with open('high_score.txt', 'r+') as file:
        saved_high_score = int(file.read().rstrip())
        if stats.score > saved_high_score:
            file.seek(0)
            file.truncate()
...
def check_aliens_bottom(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    for alien in aliens.sprites():
        if alien.rect.bottom >= screen_rect.bottom:
            alien_to_bottom(ai_settings, screen, stats, sb, ship, aliens, bullets)
            break


def ship_hit(stats, sb, ship, bullets):
    """响应飞船被外星人撞到"""
...
def alien_to_bottom(ai_settings, screen, stats, sb, ship, aliens, bullets):
    """响应外星人到达底部"""
...
def update_aliens(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    if collide_alien:
        aliens.remove(collide_alien)
        ship_hit(stats, sb, ship, bullets)

    check_aliens_bottom(ai_settings, screen, stats, sb, ship, aliens, bullets)


def update_screen(bg, ai_settings, screen, stats, sb, ship, aliens, bullets, play_button):
    """更新屏幕上的图像，并切换到新屏幕"""
